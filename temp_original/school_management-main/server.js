...
app.get('/student/fees', (req, res) => {
    ...
    db.query('SELECT id FROM students WHERE user_id = ?', [req.session.user.id], (err, std) => {
        ...
        `;
        db.query(query, [std[0].id], (err2, vouchers) => {
            // Also fetch monthly breakdown for the active voucher
            if (vouchers.length > 0) {
                db.query('SELECT * FROM fees WHERE voucher_id = ? ORDER BY id DESC', [vouchers[0].id], (err3, items) => {
...
app.get('/library', (req, res) => {
    ...
    if (role === 'student') {
        ...
        db.query(studentQuery, [userId], (err, studentResults) => {
            ...
            if (studentResults.length === 0) {
                ...
                db.query("SELECT * FROM library WHERE target_grade = 'General' AND (visibility = 'student' OR visibility = 'both') ORDER BY uploaded_at DESC", (err2, books) => {
                    res.render('library_view', { books, role: 'student', studentClass: 'No Class Assigned' });
                });
            } else {
                ...
                db.query(libraryQuery, [studentClass], (err2, books) => {
                    ...
                    res.render('library_view', { books, role: 'student', studentClass: studentClass });
                });
            }
        });
        ...
        db.query("SELECT * FROM library WHERE visibility = 'teacher' OR visibility = 'both' ORDER BY uploaded_at DESC", (err, books) => {
            if (err) console.error(err);
            res.render('library_view', { books, role });
        });
    } else if (role === 'admin') {
...
app.post('/admin/cms/settings/update', upload.single('hero_image'), csrfProtection, (req, res) => {
    if (!req.session.user || req.session.user.role !== 'admin') return res.redirect('/login');
...
app.get('/admin/cms/theme', (req, res) => {
    ...
    db.query('SELECT * FROM cms_themes WHERE is_active = TRUE LIMIT 1', (err, results) => {
        let theme = results && results.length > 0 ? results[0] : { colors: '{"primary":"#0d6efd","secondary":"#6c757d","accent":"#0dcaf0"}' };
        if (typeof theme.colors === 'string') {
            try { theme.colors = JSON.parse(theme.colors); } catch (e) { }
        }
        res.render('admin/cms/theme', { theme });
    });
});

app.post('/admin/cms/theme/update', csrfProtection, (req, res) => {
    if (!req.session.user || req.session.user.role !== 'admin') return res.redirect('/login');
    const { primary, secondary, accent, font_heading, font_body } = req.body;

    const colors = JSON.stringify({ primary, secondary, accent });
    const fonts = JSON.stringify({ heading: font_heading, body: font_body });

    db.query('UPDATE cms_themes SET colors = ?, fonts = ? WHERE is_active = TRUE', [colors, fonts], (err) => {
        if (err) console.error(err);
...
app.get('/admin/cms/pages/edit/:id', (req, res) => {
    ...
    if (req.params.id === 'new') {
        res.render('admin/cms/page_editor', { page: {} });
    } else {
        db.query('SELECT * FROM cms_pages WHERE id = ?', [req.params.id], (err, result) => {
            ...
            res.render('admin/cms/page_editor', { page: result[0] });
        });
    }
});
